document.addEventListener('DOMContentLoaded', () => {
  ...
  autoSendBtn.addEventListener('click',async()=>{
    if(!txtArea.value.trim()) return alert('Спочатку згенеруйте');
    autoSendBtn.disabled=true;
    try{
      const res=await fetch('/api/send-payment',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:txtArea.value})});
      if(!res.ok) throw new Error('Failed to send');
      toast.textContent='Надіслано в Telegram!';
      toast.classList.add('show');
      setTimeout(()=>toast.classList.remove('show'),1500);
    }catch(e){ alert(e.message); }
    finally{ autoSendBtn.disabled=false; }
  });
